...
#   pdf_extractor.py → Extração de dados de PDF (pedido/contrato), compartilhado
#                      com processos externos ao dashboard (ex: agente de e-mail)
# =============================================================================
...
# --------------------------------------------------------------------------- #
# Extração de dados de PDF — motor compartilhado em pdf_extractor.py (também  #
# usado por processos externos, ex: um agente que lê e-mails e registra       #
# pedidos automaticamente). Aqui só os wrappers que nunca escrevem nada sós:   #
# só sugerem valores para a pessoa revisar no formulário antes de confirmar.  #
# --------------------------------------------------------------------------- #

def _extrair_dados_documento_ia(pdf_bytes: bytes) -> dict | None:
    return pdf_extractor.extrair_dados_documento(pdf_bytes)


def _mesclar_dados_extraidos(docs: list[dict]) -> dict:
    return pdf_extractor.mesclar_dados_extraidos(docs)
